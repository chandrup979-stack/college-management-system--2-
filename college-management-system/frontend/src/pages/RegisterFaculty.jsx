...
const RegisterFaculty = () => {
  ...
  useEffect(() => {
    api.get("/departments").then((res) => {
      ...
      if (res.data.length === 1) setForm((f) => ({ ...f, department: res.data[0]._id }));
    });
  }, []);
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6">
      <div className="w-full max-w-lg bg-white rounded-xl shadow-sm border border-slate-100 p-8">
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="font-display font-semibold text-lg text-ink-900">Faculty Registration</span>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3">
          ...
          <input type="text" placeholder="Employee ID" value={form.employeeId} onChange={(e) => setForm({ ...form, employeeId: e.target.value })} required className="border border-slate-300 rounded-lg px-3 py-2 text-sm" />
          <input type="text" placeholder="Designation" value={form.designation} onChange={(e) => setForm({ ...form, designation: e.target.value })} className="border border-slate-300 rounded-lg px-3 py-2 text-sm" />
        ...
        <p className="text-sm text-slate-500 text-center mt-4">
          ...
          <Link to="/login/faculty" className="text-ink-900 font-medium hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
